...
```bash
python3 -m unittest discover -s backtester/tests -t .   # 531 tests
...
757 tests total, no test-framework dependency in any component (`unittest` and `node:test`).
