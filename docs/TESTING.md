...
```bash
python3 -m unittest discover -s backtester/tests -t .   # 554 tests
...
780 tests total, no test-framework dependency in any component (`unittest` and `node:test`).
