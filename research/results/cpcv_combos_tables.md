...
| 9 | `all(hurst_switch+stochastic)` | regime-filter+oscillator-reversion | **-0.040** | 2.187 | 48% | -0.1% | 90 |
...
| # | combination | families | median Sharpe | IQR | % paths + | median ret | trades |
|---|---|---|---|---|---|---|---|
| 1 | `all(sma_regime+breakout+obv_trend)` | sma-gated+breakout+volume-flow | **+0.380** | 1.880 | 52% | +9.6% | 82 |
...
| 11 | `all(sma_regime+zscore+obv_trend)` | sma-gated+mean-reversion+volume-flow | **+0.249** | 0.592 | 52% | +2.9% | 10 |
...
| 14 | `vote(sma_regime+zscore+obv_trend)` | sma-gated+mean-reversion+volume-flow | **+0.219** | 1.172 | 62% | +1.4% | 1422 |
...
| 3 | `vote(sma_regime+rsi+obv_trend)` | sma-gated+oscillator-reversion+volume-flow | **+0.589** | 1.501 | 76% | +19.0% | 1283 |
...
| 6 | `vote(sma_regime+zscore+obv_trend)` | sma-gated+mean-reversion+volume-flow | **+0.512** | 1.702 | 76% | +17.6% | 1398 |
| 7 | `all(ma_crossover+sma_regime+obv_trend)` | trend+sma-gated+volume-flow | **+0.507** | 1.397 | 81% | +18.0% | 42 |
...
| 11 | `vote(ma_crossover+ts_momentum+sma_regime)` | trend+momentum+sma-gated | **+0.420** | 1.978 | 52% | +8.1% | 415 |
...
| 15 | `vote(sma_regime+breakout+obv_trend)` | sma-gated+breakout+volume-flow | **+0.383** | 1.689 | 71% | +9.4% | 763 |
