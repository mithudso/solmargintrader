...
| Turnover (vol ÷ mcap) | **~2.33%** | derived from the row above[^zec-snapshot] |
...
**Turnover ~2.33%** ($0.20B against $8.6B, 2026-08-04) — between TRX (~1.45%) and DOGE (~3.49%).[^zec-vol] But the absolute figure is what matters here: **$200M of daily volume is small for an $8.6B asset, and it is spread across relatively few venues** (see below). Depth, not turnover ratio, is ZEC's liquidity constraint.
...
**None of the assets in this reference set are held in the local cache.** `~/dev/solmargintrader/data/` contains only `BTC_1d.csv`, `ETH_1d.csv`, `SOL_1d.csv`, `SOL_1h.csv`, `JLP_spot.csv` (checked 2026-08-04).[^zec-local]
